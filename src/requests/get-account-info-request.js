...
export class GetAccountInfoRequest extends BaseRequest {
    ...
    async perform(httpClient) {
        const query = `?filterConsoleLogin=true&sku=${Sku[this.sku]}&returningUserGameYear=${Constants.PreviousYearFull}&clientVersion=1`;
        const response = await httpClient.get(`https://${Constants.UtasHost}/ut/game/fc24/v2/user/accountinfo${query}`, {
            headers: {
