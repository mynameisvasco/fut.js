...
export class AccessTokenRequest extends BaseRequest {
    ...
    async perform(httpClient) {
        ...
        const loginResponse = await httpClient.post(loginUrl, new URLSearchParams({
        ...
        if (html.includes("Review our updated terms")) {
            const tosResponse = await httpClient.post(nextUrl, new URLSearchParams({
                _readAccept: "on",
                readAccept: "on",
                _eventId: "accept",
            }), {
        ...
        let [completeUrl] = new RegExp("https...accounts.ea.com([\\w.,@?^=%&:/~+#-]*[\\w@?^=%&/~+#-])?").exec(html) ?? [""];
        if (completeUrl === "") {
            let securityParams;
            ...
            const codeResponse = await httpClient.post(nextUrl, new URLSearchParams({
            ...
            [completeUrl] = new RegExp("https...accounts.ea.com([\\w.,@?^=%&:/~+#-]*[\\w@?^=%&/~+#-])?").exec(html) ?? [""];
